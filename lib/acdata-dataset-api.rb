class ACDataDatasetAPI
  ...
  def create_dataset(session, name, instrument_id, sample_id, files, metadata={})
    ...
    files_list, file_map = self.class.build_files_structure(files)
    dataset_json = {
      :name => name,
      :sample_id => sample_id,
  ...
  def self.build_files_structure(files)
    ...
    files.each do |f|
      if File.directory?(f)
        ...
        sequence = self.add_folder(root_path, folder, files_list, file_map, sequence)
      elsif File.file?(f)
        files_list << self.add_file(File.basename(f), sequence)
  ...
  def self.add_folder(root_path, folder, files_list, file_map, sequence)
    container = {
      "folder_root" => folder
    }
